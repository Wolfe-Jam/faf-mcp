/**
 * 🏁 WJTTC — bun migration + MCP integrity (faf-mcp)
 *
...
describe('🏁 WJTTC — bun migration + MCP integrity (faf-mcp)', () => {
  // ───────────────────────────────────────────────────────────────────────
  describe('🛑 BRAKE (1) — the migration is real & safe', () => {
    ...
    test('the server module imports without throwing', () => {
      expect(typeof FafMcpServer).toBe('function');
  ...
  describe('⚙️ ENGINE (2) — MCP protocol core', () => {
    ...
    test('getServer() exposes the SDK Server', () => {
      const s = new FafMcpServer({ transport: 'stdio', fafEnginePath: 'native' }).getServer();
